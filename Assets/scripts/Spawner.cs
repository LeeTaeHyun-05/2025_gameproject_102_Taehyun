...
public class Spawner : MonoBehaviour
{
    ...
    public GameObject coinPrefabs;       //동전 프리팹을 선언한다.
    public GameObject MissilePrefabs;    //미사일 프리팹을 선언한다.
    ...
    void Update()
    {
        timer += Time.deltaTime;       //생성 시간이 되면 오브젝트를 생성한다.
    ...
    void SpawnObject()
    {
        Transform spawnTransform = transform;     //스포너 오브젝트의 위치와 회전 값을 가져온다. (지역 변수)
