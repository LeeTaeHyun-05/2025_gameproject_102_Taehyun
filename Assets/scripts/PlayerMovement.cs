...
{
    [Header("기본 이동 설정")]
    public float moveSpeed = 5.0f;                       //이동속도 변수 설정 
    public float jumpForce = 7.0f;                       //점프의 첨 값을 준다.
    ...
    void Update()
    {
        ...
        if (movement.magnitude > 0.11)      //입력이 있을 때 회전
        {
        ...
        //점프 입력
        if (Input.GetButtonDown("Jump") && isGrounded)  //&& 두 값이 True일때, -> (Jump 버튼 {보통 스페이스 바} 와 땅 위에 있을 때)
        {
            rb.AddForce(Vector3.up * jumpForce, ForceMode.Impulse);                //위쪽으로 설정한 힘만큼 강체에 힘을 전달한다.
            isGrounded = false;                                                     //점프를 하는 순간 땅에서 떨어졌기 때문에 False라고 한다.
            realGrouned = false;
            coyoteTimeCounter = 0;
        }
    ...
    void OnCollisionEnter(Collision collision)    //충돌이 일어났을 때 호출 되는 함수
    {
    ...
    {
        if (collision.gameObject.tag == "Ground")       //충돌이 유지되는 물체의 Tag가 Ground 인 경우
        {
    ...
    void OnCollisionExit(Collision collision)           //지면에서 떨어졌는지 확인
    {
        ...
        {
            realGrouned = false;                        //지면에서 떨어졌기 때문에 flase
        }
    }
    ...
    void OnTriggerEnter(Collider other)        //트리거 영역 안에 들어왔다를 감시하는 함수
    {
        ...
        {
            coinCount++;                             //코인 변수 1 증가시킨다. ++은 1을 증가시킨다는 축약
    ...
    void DisableGlider()                 //글라이더 비활성화 함수
    {
        ...
        rb.velocity = new Vector3(rb.velocity.x, 0, rb.velocity.z);             //즉시 낙하하도록 중력 작용
    }
